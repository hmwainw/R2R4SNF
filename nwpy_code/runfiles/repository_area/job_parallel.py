...
#-------------------------------------------------------------------------------
waste_type = ['uox', 'mox', 'coex'] # long term decay heat
waste_type = ['nuex', 'ecc', 'ecm'] # short term decay heat
# also, could split up by fuel cycle!
...
for r in rock_type:
    ...
    for w in waste_type: # specify waste type...
        for n in n_wf[w]: # ... and package loading ...
            ...
            for st in storage_time: # define st  as repository array attribute
                c.update_st(st)
                tasks.append(lv.apply(calculate_area_per_package,c,r,w,n,st))
