...
heat_pipe=openmc.Material(name='heat_pipe')
heat_pipe.set_density('g/cm3', 7.25)
heat_pipe.add_element('Fe', 0.73, percent_type='wo')
heat_pipe.add_element('Cr', 0.20, percent_type='wo')
heat_pipe.add_element('Al', 0.05, percent_type='wo')
heat_pipe.add_element('Mo', 0.02, percent_type='wo')

coolant = openmc.Material(name='coolant')
coolant.set_density('g/cm3', rho/2) #0.01046)
...
small_coolant_inner = openmc.model.ZCylinder(r=0.635)
small_coolant_outer= openmc.model.ZCylinder(r=0.675)
small_coolant_cells = [openmc.Cell(fill=coolant, region=-small_coolant_inner), openmc.Cell(fill=heat_pipe, region= +small_coolant_inner & -small_coolant_outer),
                      openmc.Cell(fill=block_graphite, region=+small_coolant_outer)]
small_coolant_univ = openmc.Universe(cells=small_coolant_cells)

large_coolant_inner = openmc.model.ZCylinder(r=0.794)
large_coolant_outer = openmc.model.ZCylinder(r=0.834)
large_coolant_cells = [openmc.Cell(fill=coolant, region=-large_coolant_inner), openmc.Cell(fill=heat_pipe, region=+large_coolant_inner & - large_coolant_outer),
                      openmc.Cell(fill=block_graphite, region=+large_coolant_outer)]
